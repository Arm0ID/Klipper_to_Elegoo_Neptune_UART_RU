...
Эта инструкция также подойдет для другого одноплатника, нужно будет гуглить *pinout*.
...
#### 2.1.1 Скачаем прошивку на ПК - Способ I:
```bash
...
#### 2.1.2 Скачаем прошивку на ПК - Способ II(Если нет `scp`):

Если не получилось первым способом, можно воспользоваться программой [WinSCP](https://winscp.net/eng/download.php)
...
![](screenshots/image_2024-12-18_13-40-06.png)
* Переходим по пути к собранному BIN-арнику в правом окне: `/home/biqu/klipper/out/`
* В левом окне открываем путь куда мы хотим скачать
* Нажимаем ПКМ по файлику `klipper.bin` и выбираем `Получить` в контекстном меню.
![](screenshots/image_2024-12-18_13-45-48.png)


#### 2.2 Подготовка СД-карты
...
#### 4.2 Шаг второй - Способ II(Если не сработал первый)
* Нужно будет выключить одноплатник,
* Достать СД-карту и подключить ее в Ваш ПК,
* Открываем раздел `boot`,
* Ищем там файлик `BoardEnv.txt` и открываем,
* Заменяем в нем `console=display` на `console=serial`,
